...
public class TestVerifyCode {
	...
	public static void main(String[] args) throws InterruptedException, HttpProcessException {
		...
		String saveCodePath = "D:/valiCode.png";//保存验证码图片路径
		...
		do {
			...
			//OCR.debug(); 
			//开始Fiddler4抓包（127.0.0.1:8888）
			//http://blog.csdn.net/chaoyu168/article/details/51065644
			
			//获取验证码
			String code = OCR.ocrCode4Net(config.url(imgUrl), saveCodePath);
			
			while(code.length()!=4){//如果识别的验证码位数不等于4，则重新识别
